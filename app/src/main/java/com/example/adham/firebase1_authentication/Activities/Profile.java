...
/*
    ...
    // bluetooth connection ...
        ...
        BluetoothDevice hc05 = btAdapter.getRemoteDevice( "98:D3:71:FD:7E:19" );
        System.out.println( hc05.getName() );
        // open connection
        mSocket = null;
        try {
            mSocket = hc05.createRfcommSocketToServiceRecord( mUUID );
            System.out.println( mSocket );
            mSocket.connect();
            System.out.println( mSocket.isConnected() );
            Toast.makeText( this, "HC-05 Connected..", Toast.LENGTH_LONG ).show();
        } catch (IOException e) {
            e.printStackTrace();
        }
        ...
        public void run() {
            while (true) {
                ...
                if (sharedpreferences.getInt( value, -1 ) > 185 && sharedpreferences.getInt( value, -1 ) < 195) {
                    sms.sendTextMessage( "01093338669", null, "hello dod", null, null );
